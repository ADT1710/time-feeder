...
with conexao:
    with conexao.cursor() as cursor:
        for dia in range(diferenca):
            ...
            sql = f'insert into dim_tempo (cod_dia, dia_mes, mes_ano, data_formatada, dia_semana, semana, mes_num, mes_nome, trimestre_num, trimestre_nome, semestre_num, semestre_nome) values ("{cod_dia}","{dia_mes}","{mes_ano}","{data_formatada}","{dia_semana}","{semana}","{mes_num}","{mes_nome}","{trimestre_num}","{trimestre_nome}","{semestre_num}","{semestre_nome}")'
            cursor.execute(sql)
    conexao.commit()